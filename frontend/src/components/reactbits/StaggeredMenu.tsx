...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  // Dynamically align menu panel right edge to content area
  useEffect(() => {
    function updatePanelPosition() {
      if (!panelRef.current) return;
      const wrapper = document.querySelector('.app-content-wrapper') as HTMLElement;
      if (!wrapper) return;
      const rect = wrapper.getBoundingClientRect();
      const rightGap = window.innerWidth - rect.right;
      if (window.innerWidth > 1024) {
        panelRef.current.style.right = `${rightGap}px`;
  ...
  return (
    ...
    >
      ...
      <nav className="sm-desktop-links-bar">
        ...
        <a href="/news" className="sm-header-link">Notizie</a>
